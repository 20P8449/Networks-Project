    ...
    def run(self):
        ...
        while True:
            try:
                ...
                #   SEARCH  #
                    ...
                    if db.is_account_exist(message[1]):
                        ...
                        if db.is_account_online(message[1]):
                            ...
                            response = "user-found " + peer_info[0] + ":" + peer_info[1]
                            logging.info("Send to " + self.ip + ":" + str(self.port) + " -> " + response) 
                            self.tcpClientSocket.send(response.encode())
                        else:
                    ...
                    name = message[1]
                    admin = message[2]
                    print(f"chatroom name = {name} and RoomCreator = {admin}")
                    # chatroom-not-exist is sent to peer if the chatroom name does not exist
                    if not db.chatroom_exist(name):

                        db.addroom(name, admin)  # Add the chatroom
                    ...
                    print("Welcome!")
                    name = message[1]
                    print(f"chatroom name = {name}")

                    if db.chatroom_exist(name):

                        db.JoinRoom(name, message[2])
                        response = "join-success"
                    ...
                    room_details = db.print_users(message[1])
                ...
                logging.error(f"Thread errors: {te}")
